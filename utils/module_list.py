...
def class_stats(loader, num_classes, device):
    ### loop on train loader
    print(f'Computing class statistics for dataset')
    class_stats = torch.zeros((num_classes)).to(device)
    for n_batch, batch in enumerate(tqdm(loader)):
        _, label = batch
        n, w, h = label.shape
        label = label.to(device)
        unique_classes = torch.unique(label)
        for i in unique_classes:
            class_stats[i] += len(torch.where(label == i)[0])
    class_stats /= float(n*(n_batch+1)*w*h)
    return class_stats


def plot_class_stats(class_stats, classdict):
    import matplotlib.pyplot as plt

    # Pie chart, where the slices will be ordered and plotted counter-clockwise:
    labels = [classdict(i) for i in range(len(class_stats))]
    fig1, ax1 = plt.subplots()
    ax1.pie(class_stats.cpu().numpy(), labels=labels, autopct='%1.1f%%',
...
def print_model_complexity(model, model_name, backbone, rgbt, device):
    from thop import profile, clever_format
    if rgbt:
        input_data = torch.randn(1, 4, 512, 512).to(device)
    else:
        input_data = torch.randn(1, 3, 512, 512).to(device)
    flops, params = profile(model, inputs = (input_data, ))
    print("### Model complexity info ###")
    print(f"Model: {model_name}, use_thermal={rgbt}")
    print(f"Backbone: {backbone}")
    print(f"Number of floating point operations: {clever_format(flops)}")
    print(f"Number of params: {clever_format(params)}")
    print("#############################")
...
def compute_unsupervised_loss(predict, target, logits, strong_threshold, ignore_index=0):
    ...
    loss = F.cross_entropy(predict, target, reduction='none', ignore_index=ignore_index)
...
class ConfMatrix(object):
    ...
    def get_metrics(self):
        ...
        try:
            ...
            iu = torch.diag(h) / ((self.eps + h.sum(1) + h.sum(0) - torch.diag(h)))
